...
bool Renderer::Init(IUnknown* panelUnknown, int width, int height) {
    try {
        ...
        // 各レンダリングステージ (Passes) のインスタンス化と初期化
        ...
        {
            m_spatialDenoiserPass->SetTemporalPass(m_temporalDenoiserPass.get()); // 関連付けをバインド
...
void Renderer::RenderFrame() {
    ...
    // Jitter を有効にする必要があるかを判定 (光線追跡/Temporal Pass が有効な場合のみ)
    ...
    if (useTemporalJitter) {
        ...
        // 従来パイプライン：Jitter を適用せず、元の投影行列をそのまま使用
...
void Renderer::UploadMeshToGpu(std::shared_ptr<Mesh> mesh, int meshId) {
    ...
    for (size_t i = 0; i < numMaterials; i++) {
        ...
        metallicRoughness[i] = PrepareTextureData(
            ...
            0xFF00FF00);
        // 法線マップを読み込む、なければ平坦法線のデフォルト値を設定 (RGB = 128, 128, 255 -> 0xFFFF8080)
    ...
    // CommandList をリセットしてアップロードの準備をする
    m_ctx.ResetCommandList();
    ...
    // ========================================================
    // DXR をサポートしている場合、この Mesh の BLAS を構築する
    // ========================================================
    std::vector<ComPtr<ID3D12Resource>> scratchBuffers; // 一時バッファ、GPU 実行完了後に自然解放
    ...
    if (m_ctx.IsDxrSupported()) {
        ...
        for (size_t i = 0; i < mesh->subMeshes.size(); ++i) {
            ...
            // ★ コア修正 1：ポインタオフセットを追加！この SubMesh の範囲のみ加速構造を構築
